  ...
  return (
    <article className="card-hover group overflow-hidden rounded-2xl border border-white/[0.07] bg-panel">
      <div className={`relative aspect-[4/2.75] overflow-hidden bg-gradient-to-br ${chatbot.color}`}>
        ...
        <span className="absolute left-3 top-3 rounded-md border border-white/10 bg-black/35 px-2 py-1 text-[9px] font-black uppercase tracking-wider text-white backdrop-blur-md">{chatbot.badge}</span>
      ...
      <div className="p-4">
        <div className="mb-2 flex items-center justify-between"><span className="text-[10px] font-bold uppercase tracking-[0.14em] text-sky-400">Chatbot</span><span className="flex items-center gap-1 text-[11px] text-slate-400"><Star className="h-3 w-3 fill-cyan-400 text-cyan-400" /> {chatbot.rating} · {chatbot.sales}</span></div>
        ...
        <div className="mt-4 flex items-end gap-2 border-t border-white/[0.06] pt-4"><span className="text-sm font-black text-white">{formatPrice(chatbot.price)}</span>{chatbot.originalPrice ? <span className="text-[11px] text-slate-500 line-through">{formatPrice(chatbot.originalPrice)}</span> : null}<Link href={`/workflow/chatbot/${chatbot.slug}`} className="ml-auto text-slate-500 transition group-hover:text-sky-400" aria-label={`Xem chi tiết ${chatbot.name}`}><ArrowRight className="h-4 w-4" /></Link></div>
      </div>
    </article>
  );
}
