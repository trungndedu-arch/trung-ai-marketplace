...
export function ChatbotAiAppCatalog({ chatbots, apps, initialTab = "all" }: { chatbots: Chatbot[]; apps: Workflow[]; initialTab?: Tab }) {
  ...
  return <>
    ...
    {visible.length ? <section className="grid items-stretch gap-x-5 gap-y-5 sm:grid-cols-2 xl:grid-cols-4">{tab === "all" ? combinedItems.map((entry) => entry.kind === "chatbot" ? <ChatbotCard key={`chatbot-${entry.item.id}`} chatbot={entry.item} /> : <WorkflowCard key={`ai-app-${entry.item.id}`} workflow={entry.item} />) : isChatbot ? visibleChatbots.map((chatbot) => <ChatbotCard key={chatbot.id} chatbot={chatbot} />) : visibleApps.map((app) => <WorkflowCard key={app.id} workflow={app} />)}</section> : <section className="grid min-h-64 place-items-center rounded-2xl border border-dashed border-sky-300/20 bg-[#0B1728]/60 text-center"><div>{isChatbot ? <Bot className="mx-auto h-9 w-9 text-slate-500" /> : <WorkflowIcon className="mx-auto h-9 w-9 text-slate-500" />}<h2 className="mt-4 text-lg font-bold text-white">Chưa tìm thấy {label} phù hợp</h2><p className="mt-2 text-sm text-slate-400">Hãy thử đổi từ khóa tìm kiếm hoặc danh mục.</p><button onClick={() => { setQuery(""); setCategory("Tất cả"); }} className="mt-4 text-sm font-bold text-sky-300">Xóa bộ lọc</button></div></section>}
  </>;
}
