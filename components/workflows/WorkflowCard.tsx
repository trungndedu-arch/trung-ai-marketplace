...
export function WorkflowCard({ workflow }: { workflow: Workflow }) {
  ...
  return (
    <PromptFreeCardShell>
      <div style={{ height: "20rem" }} className="relative overflow-hidden bg-[#07111F]">
        <img src={workflow.coverImage} alt={`Ảnh bìa ${workflow.name}`} className="h-full w-full object-cover transition duration-500 group-hover:scale-[1.025]" />
      ...
      <div className="flex min-h-[13.5rem] flex-col p-4">
        ...
        <div className="mt-3 flex flex-wrap gap-1.5">{workflow.tools.slice(0, 3).map((tool) => <span key={tool} className="rounded-md bg-white/[0.06] px-2 py-1 text-[10px] font-bold text-slate-300">{tool}</span>)}</div>
        <div className="mt-auto flex items-center justify-between gap-3 pt-3"><div className="flex min-w-0 items-center gap-2"><span className={`truncate text-xs font-semibold ${workflow.isFree ? "text-cyan-300" : "text-sky-400"}`}>{priceLabel}</span>{!workflow.isFree && workflow.originalPrice ? <span className="whitespace-nowrap text-[10px] font-semibold text-slate-500 line-through">{formatPrice(workflow.originalPrice)}</span> : null}</div><Link href={detailHref} className="inline-flex h-7 shrink-0 items-center gap-1 rounded-md bg-white/[0.06] px-2 py-1 text-[10px] font-bold text-slate-300 transition hover:bg-sky-500/15 hover:text-sky-200">Xem chi tiết <ArrowRight className="h-3 w-3" /></Link></div>
      </div>
    </PromptFreeCardShell>
  );
}
