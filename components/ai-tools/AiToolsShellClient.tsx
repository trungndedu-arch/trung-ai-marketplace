...
function Sidebar({ open, close, activeModule, user }: { open: boolean; close: () => void; activeModule: "tools" | "workflows"; user: AuthUserSummary }) {
...
export function AiToolsShellClient({ children, activeModule = "tools", user }: { children: React.ReactNode; activeModule?: "tools" | "workflows"; user: AuthUserSummary }) {
  const [menuOpen, setMenuOpen] = useState(false);
  return <div className="min-h-screen bg-ink text-slate-100"><Sidebar open={menuOpen} close={() => setMenuOpen(false)} activeModule={activeModule} user={user} /><div className="lg:pl-[252px]"><Header openMenu={() => setMenuOpen(true)} user={user} />{children}</div></div>;
}
