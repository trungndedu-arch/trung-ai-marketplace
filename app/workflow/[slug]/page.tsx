...
export default async function WorkflowDetailPage({ params }: WorkflowPageProps) {
  ...
  const actionLabel = workflow.ctaLabel ?? (workflow.isFree ? "Sử dụng miễn phí" : "Mua Workflow");
  const priceLabel = workflow.isFree ? "Miễn phí" : formatPrice(workflow.price);
  const actionHref = workflow.appUrl || "#workflow-steps";
  const actionTarget = workflow.appUrl ? "_blank" : undefined;
  const paymentProduct = { id: workflow.id, name: workflow.name, slug: workflow.slug, type: "AI App", price: workflow.price, originalPrice: workflow.originalPrice, transferContent: workflow.transferContent, coverImage: workflow.coverImage };
  const ctaTitle = workflow.ctaTitle ?? (workflow.isFree ? "Bắt đầu với Workflow này ngay" : "Sở hữu Workflow để áp dụng ngay");
    ...
    <main className="mx-auto max-w-[1500px] px-4 py-9 sm:px-6 lg:px-8 lg:py-12">
      ...
      <section className="relative overflow-hidden rounded-[28px] border border-white/[0.08] bg-[#0B1728]">
        ...
        <div className="grid lg:grid-cols-[minmax(0,1.05fr)_minmax(360px,.95fr)]">
          <div className="relative aspect-[4/3] overflow-hidden bg-[#07111F]"><img src={workflow.coverImage} alt={`Ảnh bìa ${workflow.name}`} className="absolute inset-0 h-full w-full object-contain" /></div>
          <div className="relative p-6 sm:p-10"><Link href="/workflow?tab=ai-app" className="mb-7 inline-flex items-center gap-2 text-sm font-bold text-sky-300 transition hover:text-white"><ArrowLeft className="h-4 w-4" /> Quay lại AI App</Link><div className="mb-5 flex flex-wrap items-center gap-2"><span className={`rounded-full border px-3 py-1.5 text-xs font-black ${workflow.isFree ? "border-cyan-200/35 bg-cyan-400/20 text-cyan-100" : "border-sky-300/25 bg-sky-500/10 text-sky-200"}`}>{workflow.isFree ? "FREE" : workflow.badge}</span><span className="rounded-full border border-white/10 bg-white/[0.04] px-3 py-1.5 text-xs font-bold text-slate-200">{workflow.category}</span></div><h1 className="text-3xl font-extrabold tracking-[-.04em] text-white sm:text-4xl">{workflow.name}</h1><p className="mt-5 text-sm leading-7 text-slate-300 sm:text-base">{workflow.shortDescription}</p>{!workflow.hidePrice ? <div className="mt-7 flex items-end gap-3"><div><p className="text-xs font-bold uppercase tracking-[.14em] text-slate-500">Giá AI App</p><div className="mt-1 flex items-center gap-3"><span className={`text-3xl font-black ${workflow.isFree ? "text-cyan-300" : "text-white"}`}>{priceLabel}</span>{!workflow.isFree && workflow.originalPrice ? <span className="text-sm font-semibold text-slate-500 line-through">{formatPrice(workflow.originalPrice)}</span> : null}</div>{workflow.license ? <p className="mt-2 text-xs font-semibold text-slate-300">{workflow.license}</p> : null}</div></div> : null}{workflow.isFree ? <a href={actionHref} target={actionTarget} rel="noopener noreferrer" className="mt-8 inline-flex min-h-12 items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-blue-500 to-sky-500 px-5 text-sm font-extrabold text-white shadow-[0_0_26px_rgba(59,130,246,.26)] transition hover:-translate-y-0.5 hover:brightness-110">{actionLabel}<WorkflowIcon className="h-4 w-4" /></a> : <PaymentButton product={paymentProduct} className="mt-8 inline-flex min-h-12 items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-blue-500 to-sky-500 px-5 text-sm font-extrabold text-white shadow-[0_0_26px_rgba(59,130,246,.26)] transition hover:-translate-y-0.5 hover:brightness-110">{actionLabel}<WorkflowIcon className="h-4 w-4" /></PaymentButton>}</div>
        </div>
      </section>
      ...
      <section className="relative mt-14 overflow-hidden rounded-[28px] border border-sky-300/20 bg-gradient-to-br from-[#0F1F33] via-[#0B1728] to-sky-950/45 p-7 sm:p-10"><div className="absolute -right-14 -top-14 h-48 w-48 rounded-full bg-sky-400/15 blur-[70px]" /><div className="relative max-w-2xl"><p className="text-xs font-black uppercase tracking-[.2em] text-sky-300">Sẵn sàng áp dụng?</p><h2 className="mt-3 text-2xl font-extrabold tracking-tight text-white sm:text-3xl">{ctaTitle}</h2><p className="mt-3 text-sm leading-7 text-slate-300 sm:text-base">{ctaDescription}</p>{workflow.isFree ? <a href={actionHref} target={actionTarget} rel="noopener noreferrer" className="mt-6 inline-flex min-h-12 items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-blue-500 to-sky-500 px-5 text-sm font-extrabold text-white shadow-[0_0_26px_rgba(59,130,246,.26)] transition hover:-translate-y-0.5 hover:brightness-110">{actionLabel}<WorkflowIcon className="h-4 w-4" /></a> : <PaymentButton product={paymentProduct} className="mt-6 inline-flex min-h-12 items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-blue-500 to-sky-500 px-5 text-sm font-extrabold text-white shadow-[0_0_26px_rgba(59,130,246,.26)] transition hover:-translate-y-0.5 hover:brightness-110">{actionLabel}<WorkflowIcon className="h-4 w-4" /></PaymentButton>}</div></section>
