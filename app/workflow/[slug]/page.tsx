...
export default async function WorkflowDetailPage({ params }: WorkflowPageProps) {
  ...
  const actionHref = workflow.appUrl || workflow.purchaseUrl || "#workflow-steps";
  const actionTarget = workflow.appUrl || workflow.purchaseUrl ? "_blank" : undefined;
  ...
  return (
    <main className="mx-auto max-w-[1500px] px-4 py-9 sm:px-6 lg:px-8 lg:py-12">
      ...
      <section className="relative mt-14 overflow-hidden rounded-[28px] border border-sky-300/20 bg-gradient-to-br from-[#0F1F33] via-[#0B1728] to-sky-950/45 p-7 sm:p-10"><div className="absolute -right-14 -top-14 h-48 w-48 rounded-full bg-sky-400/15 blur-[70px]" /><div className="relative max-w-2xl"><p className="text-xs font-black uppercase tracking-[.2em] text-sky-300">Sẵn sàng áp dụng?</p><h2 className="mt-3 text-2xl font-extrabold tracking-tight text-white sm:text-3xl">{ctaTitle}</h2><p className="mt-3 text-sm leading-7 text-slate-300 sm:text-base">{ctaDescription}</p><a href={actionHref} target={actionTarget} rel={workflow.appUrl || workflow.purchaseUrl ? "noopener noreferrer" : undefined} className="mt-6 inline-flex min-h-12 items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-blue-500 to-sky-500 px-5 text-sm font-extrabold text-white shadow-[0_0_26px_rgba(59,130,246,.26)] transition hover:-translate-y-0.5 hover:brightness-110">{actionLabel}<WorkflowIcon className="h-4 w-4" /></a></div></section>
